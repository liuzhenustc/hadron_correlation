...
void StPicoDstMaker::fillTracks() {
  ...
  StPicoEvent *event = mPicoDst->event();
  bool keepEMu = false;
  if(event && !event->isDiMuonHFT() && !event->isSingleMuon()
     && (event->isDiMuon() || event->isEMuon()) )
    {
      LOG_DEBUG << "Keep only e+mu tracks for dimuon and emu events!" << endm;
      keepEMu = true;
    }
